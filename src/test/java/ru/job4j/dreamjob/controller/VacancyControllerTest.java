...
public class VacancyControllerTest {
    ...
    public void whenRequestVacancyListPageThenGetPageWithVacancies() {
        var vacancy1 = new Vacancy(1, "test1", now(), "desc1", true, 1, 2);
        var vacancy2 = new Vacancy(2, "test2", now(), "desc2", false, 3, 4);
    ...
    public void whenPostVacancyWithFileThenSameDataAndRedirectToVacanciesPage() throws Exception {
        var vacancy = new Vacancy(1, "test1", now(), "desc1", true, 1, 2);
    ...
    @Test
    public void whenGetVacancyByIdThenGetVacancy() {
        Vacancy vacancy = new Vacancy(1, "test1", now(), "desc1", true, 1, 2);
        ...
        var actualCities = model.getAttribute("cities");
        Vacancy currentVacancy = (Vacancy) model.getAttribute("vacancy");

        assertThat(view).isEqualTo("/vacancies/one");
        assertThat(currentVacancy).isEqualTo(vacancy);
        assertThat(actualCities).isEqualTo(expectedCities);
    }

    @Test
    public void whenUpdateVacancyThenUpdated() throws Exception {
        Vacancy vacancy = new Vacancy(1, "test1", now(), "desc1", true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenDeleteVacancyThenRedirectToVacancyList() {
        int id = 1;
        var idArgumentCaptor = ArgumentCaptor.forClass(Integer.class);
        when(vacancyService.deleteById(idArgumentCaptor.capture())).thenReturn(true);

        Model model = new ConcurrentModel();
        String view = vacancyController.delete(model, id);
